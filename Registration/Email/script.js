...
tel.oninput = function() {
    var err = document.getElementById("errMsg");
    let regist = /^(([^<>()[\].,;:\s@"]+(\.[^<>()[\].,;:\s@"]+)*)|(".+"))@(([^<>()[\].,;:\s@"]+\.)+[^<>()[\].,;:\s@"]{2,})$/iu;
    let tgg = document.getElementById("telegram");
    if (regist.test(tgg.value) == false)
    {
        err4 = true;
        err.textContent = 'Некорректное значение Email';
    }
    else
    {
        err4 =false;
        if (err2 == false && err3 == false && err1 == false)
        {
            err.textContent = '';
        }
...
function nick()
{
    ...
    {
        err1 =false;
        if (err2 == false && err3 == false && err4 == false)
        {
            err.textContent = '';
        }
        else if (err2 == true){err.textContent = 'Длина пароля должна быть не менее восьми символов';}
        else if (err4 == true){err.textContent = 'Некорректное значение Email';}
        else{err.textContent = 'Пароли не совпадают';}
    }
}
...
function pass()
{
    ...
    {
        err2 = false;
        if(err1 == false && err3 == false && err4 == false) {err.textContent = '';}
        else if (err1 == true){err.textContent = 'Длина никнейма должна быть не менее трёх и не более шестнадцати символов';}
        else if (err4 == true){err.textContent = 'Некорректное значение Email';}
        else{err.textContent = 'Пароли не совпадают';}
    ...
    {
        err3 =false;
        if(err1 == false && err2 == false && err4 == false){err.textContent = '';}
        else if (err1 == true) {err.textContent = 'Длина никнейма должна быть не менее трёх и не более шестнадцати символов';}
        else if (err4 == true){err.textContent = 'Некорректное значение Email';}
...
form.onsubmit = function(event)
{
    ...
    if(tg.length==0 || regist.test(tg.value) == false){err.textContent = 'Некорректное значение Email'; errr4=true;}
    else{errr4 == false}
