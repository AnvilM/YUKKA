...
$hn = $bd->query("SELECT * FROM `users` WHERE `vid` = '$tg'");
$bd->close();
$hn = $hn->fetch_assoc();
if (count($hn)!=0)
{
    
    $last = ' <a href="../../../Recovery/main">Восстановить пароль</a><p></p><a href="../main">Ввести другой VK ID</a></div></div>';
    $text = '<p>Данный VK ID Занят</p>';
...
{
    $text = '<p>Некорректное значение VK ID</p>';
    echo '<link type="image/x-icon" href="../../../Images/err.ico" rel="shortcut icon">';
    echo "<title>Ошибка</title>";
    echo ''.$first.''.$text.''.$last.'';
    exit;
